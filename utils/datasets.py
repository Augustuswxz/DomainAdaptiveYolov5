...
def create_dataloader(path, path_fake, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
    ...
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels(path, path_fake, imgsz, batch_size,
    ...
    # sampler = torch.utils.data.distributed.DistributedSampler(dataset, shuffle=False) if rank != -1 else None  # load data sequentially in UMT
    ...
    dataloader = loader(dataset,
                        ...
                        sampler=sampler,
                        pin_memory=True,
                        # drop_last=True,  # BUG: IndexError: index xx is out of bounds for dimension 0 with size xx
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, path_fake, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            for p, p_fake in zip(path, path_fake):
                p = Path(p)  # os-agnostic
                p_fake = Path(p_fake)  # os-agnostic
                # if p.is_dir():  # dir
                if p.is_dir() and p_fake.is_dir():  # dir
                    f += sorted(glob.glob(str(p / '**' / '*.*'), recursive=True))
                    f_fake += sorted(glob.glob(str(p_fake / '**' / '*.*'), recursive=True))
                    # f = list(p.rglob('**/*.*'))  # pathlib
                elif p.is_file() and p_fake.is_file():  # file
                    with open(p, 'r') as t:
        ...
        for img_file in img_files_new_order:
            ind = self.img_files.index(img_file)
        ...
        if self.rect:
            ...
            self.img_files_fake = [self.img_files_fake[i] for i in irect]
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            img, img_fake, labels = load_mosaic(self, index)
            shapes = None

            # MixUp https://arxiv.org/pdf/1710.09412.pdf
            if random.random() < hyp['mixup']:
                # img2, labels2 = load_mosaic(self, random.randint(0, self.n - 1))
                img2, img2_fake, labels2 = load_mosaic(self, random.randint(0, self.n - 1))
                r = np.random.beta(32.0, 32.0)  # mixup ratio, alpha=beta=32.0
                img = (img * r + img2 * (1 - r)).astype(np.uint8)
                img_fake = (img_fake * r + img2_fake * (1 - r)).astype(np.uint8)
                labels = np.concatenate((labels, labels2), 0)

        else:
            # Load image
            img, (h0, w0), (h, w) = load_image(self, index)
            img_fake, (h0, w0), (h, w) = load_image(self, index, type="fake")

            # Letterbox
            shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            img, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)
            img_fake, ratio, pad = letterbox(img_fake, shape, auto=False, scaleup=self.augment)
        ...
        if self.augment:
            ...
            augment_hsv(img, img_fake, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
        ...
        if self.augment:
            ...
            if random.random() < hyp['fliplr']:
                img = np.fliplr(img)
                img_fake = np.fliplr(img_fake)
                if nL:
                    labels[:, 1] = 1 - labels[:, 1]

        labels_out = torch.zeros((nL, 6))  # labels_out --> targets, contains (image,class,x,y,w,h)
        if nL:
            labels_out[:, 1:] = torch.from_numpy(labels)

        # Convert
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3 x img_height x img_width
        img_fake = img_fake[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3 x img_height x img_width
        img = np.ascontiguousarray(img)
    ...
    def collate_fn(batch):
        ...
        img, img_fake, label, path, path_fake, shapes = zip(*batch)  # transposed
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        # return torch.stack(img, 0), torch.cat(label, 0), path, shapes
        return torch.stack(img, 0), torch.stack(img_fake, 0), torch.cat(label, 0), path, path_fake, shapes
    ...
    def collate_fn4(batch):
        ...
        img4, img4_fake, label4, path4, path4_fake, shapes4 = [], [], [], path[:n], path_fake[:n], shapes[:n]
        ...
        return torch.stack(img4, 0), torch.stack(img4_fake, 0), torch.cat(label4, 0), path4, path4_fake, shapes4


# Ancillary functions --------------------------------------------------------------------------------------------------
def load_image(self, index, type="real"):
...
def augment_hsv(img, img_fake, hgain=0.5, sgain=0.5, vgain=0.5):
    if hgain or sgain or vgain:
        r = np.random.uniform(-1, 1, 3) * [hgain, sgain, vgain] + 1  # random gains
        hue, sat, val = cv2.split(cv2.cvtColor(img, cv2.COLOR_BGR2HSV))
        hue_f, sat_f, val_f = cv2.split(cv2.cvtColor(img_fake, cv2.COLOR_BGR2HSV))
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            ...
            img4_fake = np.full((s * 2, s * 2, img_fake.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
        ...
        img4_fake[y1a:y2a, x1a:x2a] = img_fake[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
        padw = x1a - x1b
        padh = y1a - y1b
...
def random_perspective(img, img_fake, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, 
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            img_fake = cv2.warpPerspective(img_fake, M, dsize=(width, height), borderValue=(114, 114, 114))
        else:  # affine
            img = cv2.warpAffine(img, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            img_fake = cv2.warpAffine(img_fake, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
