...
def check_dataset(data, autodownload=True): 
    return  # need to be re-write or command out [2021-12-13]
...
def check_dataset_umt(data, autodownload=True): 
    ''' need to be re-write or command out [2021-12-13] '''
    # Download dataset if not found locally
    path = Path(data.get('path', ''))  # optional 'path' field
    if path:
        # for k in 'train', 'val', 'test':
        for k in 'train_source_real', 'train_source_fake', 'train_target_real', 'train_target_fake', 'test_target_real':
            if data.get(k):  # prepend path
                data[k] = str(path / data[k]) if isinstance(data[k], str) else [str(path / x) for x in data[k]]

    train_source_real, train_source_fake, train_target_real, train_target_fake, test_target_real, s = [data.get(x) for x in (
        'train_source_real', 'train_source_fake', 'train_target_real', 'train_target_fake', 'test_target_real', 'download')]
    # train, val, test, s = [data.get(x) for x in ('train', 'val', 'test', 'download')]
